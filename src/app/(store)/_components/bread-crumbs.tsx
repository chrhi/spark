...
const Breadcrumbs: FC<BreadcrumbsAbdullahProps> = ({
  ...
  return (
    <div className="w-full h-[40px] flex items-center justify-start my-2">
      <Link href={"/"}>
        {" "}
        <span className=" text-xs lg:text-sm mx-2 text-gray-500">Home</span>
      </Link>{" "}
      &gt;{" "}
      <Link href={"/collection/hhhh"}>
        {" "}
        <span className=" text-xs lg:text-sm mx-2  text-gray-500">
          {category}
        </span>
      </Link>{" "}
      &gt;{" "}
      <span className=" text-xs lg:text-sm mx-2 font-bold text-gray-700">
